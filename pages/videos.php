<?php
	try {
		...
		$bad_params_message = "Unable to fetch videos based on the url.";
		...
		// The javascript will use these IDs to perform web service requests to fetch new videos
		$categories_arr = array(
			'all' => array('All Videos', false, 'A number of family-friendly, entertaining videos available to watch directly on Game 103.')
		);
		...
		$select_str = "SELECT * FROM (
						SELECT entries.name as name, entries.description, entries.url_name, entries.image_url, entries.rating, FORMAT(entries.views, 0)
		...
		$select_statement->bind_result($name, $description, $url_name, $image_url, $rating, $views, $total_count);
		...
		while($select_statement->fetch()) {
			...
			$views_span = "<span class = 'entry-plays'> $views $views_str</span>";
			
			$display_page .= "<a href=\"/video/$url_name\" class = 'entry-link'>
		...
		foreach($categories_arr as $key => $value) {
			...
			<li class='dropdown-item $selected' onclick='categoryAndFetch(\"$key\")'>
				<span id='$key' class='dropdown-item-text' description=\"$category_description\">
					$display
				</span>
			</li>";
		}
		...
		$max_page = ceil($total_count / $videos_per_page);
		...
		$display_title = $category_for_title . "Videos";
		...
		function fetch(changeState) {
			...
			var errorText = 'Sorry, an error occured while trying to fetch more videos. Please try again later.';
			var videos = document.getElementById('videos-entries');
				...
				if (xhttp.readyState == 4) {
					if(changeState) {
						history.pushState({
							...
							page: page,
							sort: sort
						}, '', '/videos/' + category + paramSearch + '/' + sort + '/' + page);
						// Update the title
						var keepTitle = document.title.split(' - ')[1];
						var categoryForTitle = '';
						if(category != 'all') {
							categoryForTitle = document.getElementById(category).innerHTML + ' ' + 'Videos';
						}
						else {
							categoryForTitle = 'Videos';
						}
					...
					if(xhttp.status == 200) {
						try {
							...
							if(status != 'success') {
								videos.innerHTML = object['message'];
								pagingControls.style.display = 'none';
							}
							else {
								var videosHTML = '';
								var videosArr = object['videos'];
								for(var i = 0; i < videosArr.length; i++) {
									var ratingWidth;
									ratingWidth = videosArr[i]['rating'] * 22 + 'px';
									var viewsStr;
									var viewsSpan;
									if(videosArr[i]['views'] == 1) {
										viewsStr = 'view';
									}
									else {
										viewsStr = 'views';
									}
									viewsSpan = \"<span class = 'entry-plays'> \" + videosArr[i]['views'] + ' ' + viewsStr + \"</span>\";
									var videoURL = '/video/' +  videosArr[i]['url_name'];
									videosHTML += \"<a href='\" + videoURL + \"' class = 'entry-link'> \"
									+ \"<span class = 'entry-item'> \"
									+ \"<img alt = '\" + videosArr[i]['name'] + \"' src = '\" + videosArr[i]['image_url'] + \"'> \"
									+ \"<span class = 'entry-title'>\" + videosArr[i]['name'] + \"</span> \"
									+ \"<span class='stars entry-stars'><span style='width: \" + ratingWidth + \"'></span></span> \"
									+ \"<span class = 'entry-description'> \" + videosArr[i]['description'] + \"</span> \";
									videosHTML += viewsSpan + \"</span></a>\";
								}
			...
			xhttp.open('GET', '/videos/ws/' + category + '/' + actualSearch + '/' + sort + '/' + page, true);
			xhttp.send();
		}
		...
		function generatePagingControls(count) {
			...
			if(count > 0) {
				var maxPage = Math.ceil(count / $videos_per_page);
