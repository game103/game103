<?php
	$error_val = 'Sorry, there was an error loading similar videos. Please try again later.';
	...
	try {
		...
		$random_select_str = "SELECT entries.name, entries.description, entries.url_name, entries.image_url, entries.rating, FORMAT(entries.views, 0)
		...
		$random_select_statement->bind_result($random_name, $random_description, $random_url_name, $random_image_url, $random_rating, $random_views);
		while($random_select_statement->fetch()) {
			...
			if($random_views == 1) {
				$random_views_str = 'view';
			}
			else  {
				$random_views_str = 'views';
			}
			...
			<span class = 'entry-plays'> $random_views $random_views_str</span>
			</span>
			</a>";
		}
